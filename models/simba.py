...
class SIMBA(nn.Module):
    ...
    def __init__(self, num_classes=1, aux_logits=False, transform_input=False,
        ...
        for x in range(2):
            self.Conv2d_1a_3x3.append(BasicConv2d(1, 32, kernel_size=3, stride=2, padding=0))
            self.Conv2d_2a_3x3.append(BasicConv2d(32, 32, kernel_size=3))
            self.Conv2d_2b_3x3.append(BasicConv2d(32, 64, kernel_size=3, padding=1))
            self.Conv2d_3b_1x1.append(BasicConv2d(64, 80, kernel_size=1))
            self.Conv2d_4a_3x3.append(BasicConv2d(80, 192, kernel_size=3))
...
class InceptionC(nn.Module):
    ...
    def forward(self, x):
        ...
        b = self.branch7x7dbl_2(b); b = self.branch7x7dbl_3(b)
        b = self.branch7x7dbl_4(b); branch7x7dbl = self.branch7x7dbl_5(b)
        branch_pool = self.branch_pool(F.avg_pool2d(x, kernel_size=3, stride=1, padding=1))
...
class InceptionD(nn.Module):
    ...
    def forward(self, x):
        ...
        b = self.branch7x7x3_1(x); b = self.branch7x7x3_2(b)
        b = self.branch7x7x3_3(b); branch7x7x3 = self.branch7x7x3_4(b)
        branch_pool = F.max_pool2d(x, kernel_size=3, stride=2)
...
class InceptionE(nn.Module):
    ...
    def forward(self, x):
        ...
        branch3x3 = torch.cat([self.branch3x3_2a(b3), self.branch3x3_2b(b3)], 1)
        bdbl = self.branch3x3dbl_2(self.branch3x3dbl_1(x))
        branch3x3dbl = torch.cat([self.branch3x3dbl_3a(bdbl), self.branch3x3dbl_3b(bdbl)], 1)
        branch_pool = self.branch_pool(F.avg_pool2d(x, kernel_size=3, stride=1, padding=1))
